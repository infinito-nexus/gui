...
If a `docker pull` or general image pull fails, it may be caused by an MTU, network, or IPv6 issue on the host machine.
...
Disable IPv6 only in Docker via `/etc/docker/daemon.json`:
