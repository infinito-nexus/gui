...
- To smoke-test the deployment endpoint end-to-end (CSRF prime → workspace create → inventory → POST `/api/deployments`) against the running stack from inside the docker-compose network — useful when the host shell cannot reach `127.0.0.1:8000` (sandboxed/network-namespaced terminals) and you need to read the actual response body of a 500 — you MUST use `make api-smoke-deployment` instead of running raw `docker run --network ... alpine sh -c 'curl ...'` pipelines by hand. Override the target host or playbook with `make api-smoke-deployment HOST=<alias> PLAYBOOK=<path>`.
