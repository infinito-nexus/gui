...
source_repo_root="${{JOB_RUNNER_REPO_DIR:-{shlex.quote(cfg.repo_dir)}}}"
...
workspace_inventory={shlex.quote(workspace_inventory)}
...
"""
    ...
    for source, target, is_read_only in bind_mounts or []:
        host_source = resolve_host_mount_source(source)
    ...
    *,
    ...
    for index, arg in enumerate(lowered_args):
        if not arg:
            continue
        lower = arg.lower()
        if lower in _DISALLOWED_HARDENED_FLAGS:
            raise HTTPException(
                status_code=500,
        ...
        if any(
            ...
            for flag in _DISALLOWED_HARDENED_FLAGS
            if flag != "--privileged"
        ):
            raise HTTPException(
                status_code=500,
...
def remove_container(container_name: Optional[str]) -> None:
    name = (container_name or "").strip()
    if not name:
        return
    try:
...
def create_tmpfs_volume(
    ...
    *,
    ...
    if not name:
        return
    size_value = max(int(size_mebibytes), 1)
    result = subprocess.run(
        [
            ...
            "device=tmpfs",
            "--opt",
            f"o=size={size_value}m,uid={uid},gid={gid},mode={mode},noexec,nosuid,nodev",
            name,
        ],
...
def remove_volume(
    ...
    *,
    ...
    while True:
        try:
            result = subprocess.run(
                [resolve_docker_bin(), "volume", "rm", name],
                stdout=subprocess.DEVNULL,
                stderr=subprocess.PIPE,
                text=True,
                check=False,
...
def create_internal_network(network_name: Optional[str]) -> None:
...
def remove_network(network_name: Optional[str]) -> None:
    name = (network_name or "").strip()
    if not name:
        return
    try:
...
def inspect_container_labels(container_name: Optional[str]) -> Dict[str, str]:
    name = (container_name or "").strip()
    if not name:
        return {}
    try:
